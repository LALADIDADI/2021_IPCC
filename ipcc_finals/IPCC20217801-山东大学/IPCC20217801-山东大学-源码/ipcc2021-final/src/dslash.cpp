...
void DslashEE_inv(lattice_fermion &src, lattice_fermion &dest, const double mass)
{
    ...
    for (int t = 0; t < subgrid[3]; t++ ) {
        ...
        for (int z = 0; z < subgrid[2]; z++) {
            ...
            for (int y = 0; y < subgrid[1]; y++) {
				...
				//int offsetIndex = subgrid[0] * subgrid[1] * subgrid[2] * t + subgrid[0] * subgrid[1] * z + subgrid[0] * y;
                for (int x = 0; x < subgrid[0]; x++){
                    ...
                    AOy = U.A[1] +
                         (subgrid[0] * subgrid[1] * subgrid[2] * t + subgrid[0] * subgrid[1] * z +
                          subgrid[0] * b_y + x + (1 - cb) * subgrid_vol_cb) *
                             9;
                    AEz = U.A[2] + 
                         (subgrid[0] * subgrid[1] * subgrid[2] * t +
                         subgrid[0] * subgrid[1] * z + subgrid[0] * y + x + cb * subgrid_vol_cb) *
                             9;
                    AOz = U.A[2] +
                    ...
                    for (int c1 = 0; c1 < 3; c1++) {
                        for (int c2 = 0; c2 < 3; c2++) {
                            if(x == (subgrid[0]-1) && x_u == (subgrid[0]-1)){
                                //do nothing 
                            }else{
                                //x_f
                                tmp = -(srcO[0 * 3 + c2 - x*12 + f_x*12] - flag * I * srcO[3 * 3 + c2 - x*12 + f_x*12]) * half * AEx[c1 * 3 + c2];
                                destE[0 * 3 + c1] += tmp;
                                destE[3 * 3 + c1] += flag * (I * tmp);
                                tmp = -(srcO[1 * 3 + c2 - x*12 + f_x*12] - flag * I * srcO[2 * 3 + c2 - x*12 + f_x*12]) * half * AEx[c1 * 3 + c2];
                                destE[1 * 3 + c1] += tmp;
                            ...
                            if(x == 0 && x_d == 1){
                                ...
                                //x_d
                                tmp = -(srcO[0 * 3 + c2 - x*12 + b_x*12] + flag * I * srcO[3 * 3 + c2 - x*12 + b_x*12]) * half * conj(AOx[c2 * 3 + c1]);
                                destE[0 * 3 + c1] += tmp;
                                destE[3 * 3 + c1] += flag * (-I * tmp);
                                tmp = -(srcO[1 * 3 + c2 - x*12 + b_x*12] + flag * I * srcO[2 * 3 + c2 - x*12 + b_x*12]) * half * conj(AOx[c2 * 3 + c1]);
                                destE[1 * 3 + c1] += tmp;
                                destE[2 * 3 + c1] += flag * (-I * tmp);
                            } 
                            if(y == (subgrid[1]-1) && y_u == (subgrid[1]-1)){
                                //do nothing
                            }else{
                                //y_f                                
                                tmp = -(srcO[0 * 3 + c2 - subgrid[0]*y*12 + subgrid[0]*f_y*12] + flag * srcO[3 * 3 + c2 - subgrid[0]*y*12 + subgrid[0]*f_y*12]) * half * AEy[c1 * 3 + c2];
                                destE[0 * 3 + c1] += tmp;
                                destE[3 * 3 + c1] += flag * (tmp);
                                tmp = -(srcO[1 * 3 + c2 - subgrid[0]*y*12 + subgrid[0]*f_y*12] - flag * srcO[2 * 3 + c2 - subgrid[0]*y*12 + subgrid[0]*f_y*12]) * half * AEy[c1 * 3 + c2];
                                destE[1 * 3 + c1] += tmp;
                            ...
                            if(y == 0 && y_d == 1){
                                ...
                                //y_b
                                tmp = -(srcO[0 * 3 + c2 - subgrid[0]*y*12 + subgrid[0]*b_y*12] - flag * srcO[3 * 3 + c2 - subgrid[0]*y*12 + subgrid[0]*b_y*12]) * half * conj(AOy[c2 * 3 + c1]);
                                destE[0 * 3 + c1] += tmp;
                                destE[3 * 3 + c1] -= flag * (tmp);
                                tmp = -(srcO[1 * 3 + c2 - subgrid[0]*y*12 + subgrid[0]*b_y*12] + flag * srcO[2 * 3 + c2 - subgrid[0]*y*12 + subgrid[0]*b_y*12]) * half * conj(AOy[c2 * 3 + c1]);
                                destE[1 * 3 + c1] += tmp;
                                destE[2 * 3 + c1] += flag * (tmp);
                            }
                            if(z == (subgrid[2]-1) && z_u == (subgrid[2]-1)){
                                //do nothing
                            }else{
                                //z_f
                                tmp = -(srcO[0 * 3 + c2 - subgrid[0]*subgrid[1]*z*12 + subgrid[0]*subgrid[1]*f_z*12] - flag * I * srcO[2 * 3 + c2 - subgrid[0]*subgrid[1]*z*12 + subgrid[0]*subgrid[1]*f_z*12]) * half * AEz[c1 * 3 + c2];
                                destE[0 * 3 + c1] += tmp;
				destE[2 * 3 + c1] += flag * (I * tmp);
				tmp = -(srcO[1 * 3 + c2 - subgrid[0]*subgrid[1]*z*12 + subgrid[0]*subgrid[1]*f_z*12] + flag * I * srcO[3 * 3 + c2 - subgrid[0]*subgrid[1]*z*12 + subgrid[0]*subgrid[1]*f_z*12]) * half * AEz[c1 * 3 + c2];
				destE[1 * 3 + c1] += tmp;
				destE[3 * 3 + c1] += flag * (-I * tmp);
			    }
			    if(z == 0 && z_d == 1){
				...
				//z_b
				tmp = -(srcO[0 * 3 + c2 - subgrid[0]*subgrid[1]*z*12 + subgrid[0]*subgrid[1]*b_z*12] + flag * I * srcO[2 * 3 + c2 - subgrid[0]*subgrid[1]*z*12 + subgrid[0]*subgrid[1]*b_z*12]) * half * conj(AOz[c2 * 3 + c1]);
				destE[0 * 3 + c1] += tmp;
				destE[2 * 3 + c1] += flag * (-I * tmp);
				tmp = -(srcO[1 * 3 + c2 - subgrid[0]*subgrid[1]*z*12 + subgrid[0]*subgrid[1]*b_z*12] - flag * I * srcO[3 * 3 + c2 - subgrid[0]*subgrid[1]*z*12 + subgrid[0]*subgrid[1]*b_z*12]) * half * conj(AOz[c2 * 3 + c1]);
				destE[1 * 3 + c1] += tmp;
				destE[3 * 3 + c1] += flag * (I * tmp);
			    }
                            if(t == (subgrid[3]-1) && t_u == (subgrid[3]-1)){
                                //do nothing
			    }else{
				//t_f
				tmp = -(srcO[0 * 3 + c2 - subgrid[0]*subgrid[1]*subgrid[2]*t*12 + subgrid[0]*subgrid[1]*subgrid[2]*f_t*12] - flag * srcO[2 * 3 + c2 - subgrid[0]*subgrid[1]*subgrid[2]*t*12 + subgrid[0]*subgrid[1]*subgrid[2]*f_t*12]) * half * AEt[c1 * 3 + c2];
				destE[0 * 3 + c1] += tmp;
				destE[2 * 3 + c1] -= flag * (tmp);
				tmp = -(srcO[1 * 3 + c2 - subgrid[0]*subgrid[1]*subgrid[2]*t*12 + subgrid[0]*subgrid[1]*subgrid[2]*f_t*12] - flag * srcO[3 * 3 + c2 - subgrid[0]*subgrid[1]*subgrid[2]*t*12 + subgrid[0]*subgrid[1]*subgrid[2]*f_t*12]) * half * AEt[c1 * 3 + c2];
				destE[1 * 3 + c1] += tmp;
				destE[3 * 3 + c1] -= flag * (tmp);
			    }
			    if(t == 0 && t_d == 1){
				...
				//t_b
				tmp = -(srcO[0 * 3 + c2 - subgrid[0]*subgrid[1]*subgrid[2]*t*12 + subgrid[0]*subgrid[1]*subgrid[2]*b_t*12] + flag * srcO[2 * 3 + c2 - subgrid[0]*subgrid[1]*subgrid[2]*t*12 + subgrid[0]*subgrid[1]*subgrid[2]*b_t*12]) * half * conj(AOt[c2 * 3 + c1]);
				destE[0 * 3 + c1] += tmp;
				destE[2 * 3 + c1] += flag * (tmp);
				tmp = -(srcO[1 * 3 + c2 - subgrid[0]*subgrid[1]*subgrid[2]*t*12 + subgrid[0]*subgrid[1]*subgrid[2]*b_t*12] + flag * srcO[3 * 3 + c2 - subgrid[0]*subgrid[1]*subgrid[2]*t*12 + subgrid[0]*subgrid[1]*subgrid[2]*b_t*12]) * half * conj(AOt[c2 * 3 + c1]);
				destE[1 * 3 + c1] += tmp;
				destE[3 * 3 + c1] += flag * (tmp);
			    }
                        }
...
void DslashEE_origin(lattice_fermion &src, lattice_fermion &dest, const double mass)
{
...
void DslashOO_origin(lattice_fermion &src, lattice_fermion &dest, const double mass)
{
...
// cb = 0  EO  ;  cb = 1 OE
void Dslashoffd_origin(lattice_fermion &src, lattice_fermion &dest, lattice_gauge &U, const bool dag,
                int cb)
{
    // sublattice
    int N_sub[4] = {src.site_vec[0] / src.subgs[0], src.site_vec[1] / src.subgs[1],
