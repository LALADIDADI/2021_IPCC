...
int check(lattice_fermion &src, lattice_fermion &chi, lattice_gauge &U, double mass)
{
    ...
    // Mdb = M^\dagger b
    Dslash_origin(src, Mb, U, mass, false);
    Dslash_origin(src, Mdb, U, mass, true);
    Dslash_origin(Mb, MMb, U, mass, true);

    // rsd2 = |Mdx - b| := | M^\dagger x -b|
    Dslash_origin(chi, Mdx, U, mass, true);
    Minus(Mdx, src, tmp);
    double rsd2 = sqrt(norm_2(tmp));
    // rsd1 = |Mx - b|;
    Dslash_origin(chi, Mx, U, mass, false);
    Minus(Mx, src, tmp);
    double rsd1 = sqrt(norm_2(tmp));
    // rsd = |M\dagger M x - Mdb|;
    Dslash_origin(Mx, MMx, U, mass, true); // MMx = M^\dagger M x;
    Minus(MMx, Mdb, tmp);
